...
- `Administration__InitialAdminEmails__0` and further indexed values for bootstrap administrators
...
Bootstrap admin email settings identify existing accounts only. They are not credentials: startup adds `Admin` idempotently, never creates an account, and never removes an existing admin role when an address is later removed. A newly assigned administrator needs a fresh sign-in to receive the role claim.
