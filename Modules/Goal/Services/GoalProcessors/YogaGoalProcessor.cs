...
public class YogaGoalProcessor : IWorkoutGoalProcessor
{
    private readonly IGoalRepository _goalRepository;

    public string SupportedWorkoutType => "yoga";

    public YogaGoalProcessor(IGoalRepository goalRepository)
    {
    ...
    {
        var workout = await _goalRepository.GetYogaWorkoutByIdAsync(workoutEvent.WorkoutId, cancellationToken);
        if (workout == null) return;

        var goals = await _goalRepository.GetActiveGoalsByWorkoutTypeAsync(workoutEvent.UserId, "yoga", cancellationToken);
    ...
    private double CalculateDelta(UserGoal goal, backend.Modules.Workout.Domain.Entities.YogaUserWorkoutDetails workout)
    {
